...
def main():
    ...
    for (time, num_pods), (_, cpu_seconds_per_second), in zip(data['data']['result'][0]['values'], data2['data']['result'][0]['values']):
        new_data['data'].append([time - args.start_time, int(num_pods), float(cpu_seconds_per_second)])
